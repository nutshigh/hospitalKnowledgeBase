...
function renderConclusionText(text: string, redSentences: string[]) {
  ...
  const inline = (s: string): ReactNode => {
    ...
    return s.split(redRe).map((p, i) =>
      reds.includes(p)
        ? <span key={i} style={{ color: 'var(--color-red)', fontWeight: 600 }}>{p}</span>
        : <span key={i}>{p}</span>
    );
  };
  return text.split('\n').map((line, i) => {
...
export default function ReportDetailPage() {
  ...
  const conclusionItems = rawIndicators.filter((ind: any) => isConclusionIndicator(ind));
  if (conclusionItems.some((it: any) => it.color_level === 'red')) levelCounts.red += 1;
      ...
      <div style={{
        ...
        {conclusionExpanded && (
          <div style={{ marginTop: 12, fontSize: 14, lineHeight: 1.8, whiteSpace: 'pre-wrap', color: 'var(--color-text)',
            ...
            {conclusionText
              ? renderConclusionText(conclusionText, redConclusionSentences)
              : '未提取到结论，请尝试重新上传'}
          </div>
        )}
      </div>
