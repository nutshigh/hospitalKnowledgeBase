"""H004 报告级三件套基线护栏(2026-09-12, DB 验收快照口径)。

基线 = **用户验收过的 DB 结果快照**(`scripts/gen_h004_baseline.py` 从 DB 导出)。
...
DB = "hospital_H004"
BASELINE = json.loads(
    (BACKEND / "tests" / "modules" / "report" / "baselines" / "h004_baseline.json")
...
def test_indicator_guard(rid, conn):
    ...
    cur = conn.cursor()
    act = _actual(cur, rid)
    assert act, f"H004-{rid} {entry['name']} 无 completed 解读"
    ...
    assert not extra and not missing, (
        f"H004-{rid} {entry['name']} 指标黄红与验收基线漂移\n  多: {extra}\n  少: {missing}")
...
def test_conclusion_text_guard(rid, conn):
    ...
    assert act["conclusion_hash"] == entry["conclusion_hash"], (
        f"H004-{rid} {entry['name']} 总结段变化(长度 {act['conclusion_len']} "
...
def test_conclusion_items_guard(rid, conn):
    ...
    missing = [n for n, c in want if (n, c) not in got]
    assert not extra and not missing, (
        f"H004-{rid} {entry['name']} 总检异常与验收基线漂移\n  多: {extra}\n  少: {missing}")
