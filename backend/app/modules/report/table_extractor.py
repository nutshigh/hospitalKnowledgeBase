...
_SKIP_NAMES = re.compile(
    r"^(姓名|性别|年龄|体检日期|检查日期|审核日期|健康档案号|体检编号|登记号|电话|话$|建议$|弃检|未检|拒检|放弃检查|身份证|证件号码|\d{0,3}岁|体检次数|"
...
# 2026-09-12(用户口径): 判异常看**标志/提示列** —— 有 "±" 符号才算异常;
# "弱阳性" 是结果值描述, 标记列非 ± 时不判异常(池州维生素C 的标志列是 ± 才黄)。
